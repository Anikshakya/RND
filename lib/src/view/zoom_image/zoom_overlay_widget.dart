...
class ZoomOverlay extends StatefulWidget {
  ...
  /// Specifies whether the zoom is enabled only with two fingers on the screen.
  ///  Defaults to false.
...
class ZoomOverlayState extends State<ZoomOverlay> with TickerProviderStateMixin {
  Matrix4? _matrix = Matrix4.identity();
  ...
  final _transformWidgetKey = GlobalKey<TransformWidgetState>();
  ...
  void initState() {
    ...
    _controllerReset
      ..addListener(() {
        _transformWidgetKey.currentState?.setMatrix(_animationReset.value);
      })
  ...
  void onScaleStart(ScaleStartDetails details) {
    // Don't start the effect if the image hasn't reset completely.
    ...
    // create a matrix of where the image is on the screen for the overlay
  ...
  void onScaleUpdate(ScaleUpdateDetails details) {
    ...
    _transformWidgetKey.currentState?.setMatrix(_matrix);
  }
